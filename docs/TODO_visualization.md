...
- [ ] **手测清单（回归）**  
  ...
  - ~~提交本轮未入库改动~~（核查时 git 已干净，已勾掉）  

---

## P1 — 规格里标为增强 / 未做完

> **多文件相关项按用户要求整体 out-of-scope**，独立分支 `origin/feat/multi-file-project-run` 实验保留，不在主线推进。

- ~~**多文件真正合编译运行**~~（多文件 out-of-scope，2026-08）
- [ ] **Coze UML 稳定生成**  
  M4 静态兜底已有；AI `intent=uml` 失败率 / 缓存策略可再压一版。
- ~~**多文件编辑器可编辑 + diff**~~（多文件 out-of-scope，2026-08）
- ~~**跨模式共享运行状态**~~（多文件 out-of-scope，2026-08）
...
1. P0 手测 + 按需 commit  
2. **快排分区样图对齐**（`SortArrayCanvas` + `sortVizExtract`；l/r 边界带、pivot 区、未处理区弱化）  
3. **堆排联动高亮**（`DataStructureTab` 内数组视图 ↔ 堆树视图同一 `i/j`）  
4. TraceEngine 单源维护，减少再出「类型写死」类 bug  

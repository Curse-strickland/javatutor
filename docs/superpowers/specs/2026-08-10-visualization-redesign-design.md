...
**日期：** 2026-08-10
**状态：** Approved — 待进入实现规划
**设计来源：** 用户提供的「可视化1.pdf」「可视化2.pdf」草图 + 现有 `LinkedListCanvas` / `RecursionStackCanvas` / `SvgAnimatePanel` 基础
...
├─────────────────────────────────────────┤
│ 复杂度分析                                │  ← 现有 ControlFlowPanel
├─────────────────────────────────────────┤
│ 智能体问答                                │  ← 现有 AiTutorPanel（从问答 tab 移过来一份）
└─────────────────────────────────────────┘
```
「问答」tab 同时保留独立入口，避免破坏现有用户使用习惯。

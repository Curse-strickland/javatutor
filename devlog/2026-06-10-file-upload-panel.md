...
- 右侧"文件"标签改为"经典"，替换为 ClassicCodePanel
- 5 大类 14 个经典算法：排序(冒泡/选择/插入/快速)、查找(二分/线性)、递归与数学(斐波那契/阶乘/GCD/质数)、数据结构(链表反转/二叉树/栈括号匹配/队列约瑟夫环)、动态规划(爬楼梯/Kadane/0-1背包)
- 每类独立卡片（`border` + `border-radius: 10px`），蓝色圆点标题 + chevron 折叠/展开
- 点击条目直接加载到编辑器 + 记录到加载历史

## Bug 修复记录 (续)

### B11 — 右侧滚动栏切步骤后跳回顶部
- **根因**: `scrollIntoView({ block: 'center' })` 强制居中
- **修复**: 改为 `block: 'nearest'`

### B12 — 经典代码加载出现在上传记录中
- **根因**: 经典代码和文件上传共用同一个处理器
- **修复**: 分离 `onFileLoad`(记录历史) / `onClassicLoad`(不记录) → 后改为统一记录，上传记录→加载记录

### B13 — AI 按钮图标眼睛 hover 不变色
- **根因**: SVG circle `fill="var(--bg)"` 硬编码页面背景
- **修复**: 改为 `fill="var(--card-bg)"` + CSS 规则 hover/active 切 `var(--accent-bg)`

### B14 — 上传面板底部矩形不协调
- **修复**: `.upload-panel-wrapper` 加 `border-radius: 0 0 12px 12px`

### B15 — 新批次待加载文件挤掉旧批次
- **修复**: 追加而非替换，同名去重；"本次导入" → "已导入"

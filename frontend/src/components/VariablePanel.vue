...
const linkedListGroups = computed(() => {
  ...
  // Collect all linked-list-typed nodes from heap (val + next present)
  const allNodeRefs = new Set()
  for (const ref of Object.keys(h)) {
    const entry = h[ref]
    if (entry.hasOwnProperty('val') && entry.hasOwnProperty('next')) {
      allNodeRefs.add(ref)
    }
  }
  ...
  for (const ref of allNodeRefs) {
    const entry = h[ref]
    if (entry.next && allNodeRefs.has(entry.next)) {
      pointedTo.add(entry.next)
    }
  }
...
watch(recursionStack, (stack) => {
  if (stack && stack.frames && stack.frames.length > 0) {
    // Stack is alive: keep it in live state
    liveRecursionStack.value = stack
  } else if (!stack || !stack.frames || stack.frames.length === 0) {
    // Stack is empty → remove from live state (recursion finished)
    liveRecursionStack.value = null
  }
})
